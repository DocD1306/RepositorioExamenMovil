...
import net.iessochoa.diegovalencia.repositorioexamenmovil.tarjetaspokemonpropio.composables.DynamicSelectTextField
import net.iessochoa.diegovalencia.repositorioexamenmovil.ui.theme.Typography


@Composable
fun RickScreen(viewModel: RickViewModel = viewModel(), modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Text("Rick Retrofit", style = Typography.headlineMedium)
        ...
        Button(
            onClick = {
                // Llama al ViewModel
                viewModel.getCharactersByPage(selectedGen)
            },
        ...
        LazyVerticalGrid(
            ...
            items(items = characterList) { rick ->
                RickItem(rick)
            }
        }
    }
...
@Composable
fun RickItem(rickItem: CharacterDto) {
    Card(
...
//            val separado = rickItem.url.split("/")
//            val id = separado.get(separado.size -2)

            val imageUrl = rickItem.image // <-- IMPLEMENTAR AQUÍ LOGICA

            AsyncImage(
                model = imageUrl,
                contentDescription = rickItem.name,
                modifier = Modifier.size(100.dp)
            )

            Text(
                text = rickItem.name.uppercase(),
